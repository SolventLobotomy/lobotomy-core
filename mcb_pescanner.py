    ...
    def collect(self):
        ...
        for file in self.files:
            ...
            if len(callbacks):
                ...
                for cb in callbacks:
                    # Begin edit: w2k8
                    # got error in some images: TypeError: %x format: a number is required, not NoneType
                    try:
                        out.append("    0x%x" % cb)
                    except TypeError:# %x format: a number is required, not NoneType
                        pass
                    # End edit: w2k8
...
if __name__ == "__main__":
    ...
    peid = Lobotomy.yararules + 'peid.txt'
    ...
    # pescan.collect()
    pescan = PEScanner(files, yara_rules, peid)
    ...
    sql_prefix = "INSERT INTO pe_scanner VALUES (0, "

    # pefile.PEFormatError: "data at RVA can't be fetched. Corrupt header?"
    if "data at RVA can't be fetched. Corrupt header?" in data:
        data = "pefile.PEFormatError: data at RVA can't be fetched. Corrupt header?"
    if data != '':
        data = Lobotomy.escchar(data)
        # data = data.replace("'", '"').replace("`", "\`").replace('"', '\\"')
        sql_line = sql_prefix + "'{}', '{}''".format(object, data + "')")[:-2]
        try:
            # print sql_line
