...
def start(database, folder):
    ...
    yara_rules = Lobotomy.yararules + 'index.yar'
    ...
    if DEBUG:
        Lobotomy.pl("Write log: {}, Start: {}".format(database, command))
        Lobotomy.pl("Write log: {}, Start: {}".format(casedir, command))
    else:
    ...
    if folder == '':
        ...
        for subdir, dirs, files in os.walk(casedir):
            for folders in dirs:
                for subdir1, dirs1, files1 in os.walk(subdir + '/' + folders):
                    Lobotomy.pl("Running yarascan on folder: {}".format(subdir1))
                    for file in files1:
                        filename = ''
                        offset = ''
                        description = ''
                        ...
                        try:
                            f = open('{}-{}.txt'.format(imagename, plugin), 'a')
                            f.write(log)
                            if log != '':
                                f.write('\n')
                            f.close()
                        ...
                        try:
                            ...
                            Lobotomy.pl("Yarascan - Percentage done: {}".format(pct))
                        except:
                            pass
                        Lobotomy.pl("Yarascan - Files to go: {} to {} ".format(str(count), str(counter)))
                        Lobotomy.pl("Yarascan - Current filename: {}".format(filename))
                        if log != '':
                            for item in log.split('\n'):
                                if filename in item:
                                    ...
                                    if 'description' in item:
                                        ...
                                        filename = "/{}".format(item[start:].split('] /', 1)[1].replace('//', '/'))
                                    ...
                                    try:
                                        ...
                                        string = ''
                                    try:
                                        description = Lobotomy.escchar(description)
                                        yara_description = Lobotomy.escchar(yara_description)
                                        ...
                                        Lobotomy.pl('Error sql query: {} - {}'.format(sql_line,database))

    command = "yara {} {} -m -s -w".format(yara_rules, imagename)
    # Yara -w: disable warnings
    # Yara -m: print metadata.
    # Yara -s: print matching strings.
    Lobotomy.pl("Running Yarascan on memorydump: {}".format(imagename))
