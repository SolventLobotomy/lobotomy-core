    ...
    def parse_service_key(self, service_key):
        ...
        if "svchost.exe -k" in image_path:
            ...
            if sk:
                ...
                if main:
                    entry += " ({})".format(main)
                # Start: Edited by: w2k8 due to error on some images
                try:
                    entry = entry.replace('\x00', '')
                except AttributeError: # 'NoneType' object has no attribute 'replace'
                    pass
                # End: Edited by: w2k8 due to error on some images
